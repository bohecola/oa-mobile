<template>
  <van-form ref="Form" v-loading="isLoading && showLoading" class="reset-label" label-width="auto" label-align="top" readonly>
    <van-cell-group inset class="!my-3">
      ...
      <van-field v-if="form.recruitNo" v-model="form.recruitNo" v-show-field="['recruitNo', includeFields]" name="recruitNo" label="招聘申请" input-align="left" />

      <van-field v-show-field="['postId', includeFields]" name="postId" label="岗位名称" input-align="left">
        <template #input>
          <PostSelect v-model="form.postId" :dept-id="form.deptId" readonly />
        </template>
      </van-field>

      <van-field v-model="form.name" v-show-field="['name', includeFields]" name="name" label="姓名" input-align="left" />
      <van-field v-model="form.sex" v-show-field="['sex', includeFields]" name="sex" label="用户性别" input-align="left">
        <template #input>
      ...
      <van-field v-model="form.phonenumber" v-show-field="['phonenumber', includeFields]" name="phonenumber" label="手机号码" input-align="left" />

      <van-field v-model="form.status" v-show-field="['status', includeFields]" name="status" label="录用情况" input-align="left">
        <template #input>
          <dict-tag :options="oa_employment_status" :value="form.status" />
        </template>
      </van-field>

      <van-field v-model="form.interviewWay" v-show-field="['interviewWay', includeFields]" name="interviewWay" label="面试形式" input-align="left">
        <template #input>
          <dict-tag :options="oa_interview" :value="form.interviewWay" />
        </template>
      </van-field>

      <van-field v-model="form.interviewDate" v-show-field="['interviewDate', includeFields]" name="interviewDate" label="面试日期" input-align="left">
        <template #input>
          {{ parseTime(form.interviewDate, '{y}-{m}-{d}') }}
        </template>
      </van-field>

      <van-field v-if="form.status === '3'" v-model="form.isOwnerInterview" v-show-field="['isOwnerInterview', includeFields]" name="isOwnerInterview" label="是否需要业主面试" input-align="left">
        <template #input>
      ...
      <van-field v-model="form.certificates" v-show-field="['certificates', includeFields]" name="certificates" label="持证情况" input-align="left">
        <template #input>
          <DictSelect v-model="form.certificates" dict-type="oa_document_type" readonly />
        </template>
      </van-field>

      <van-field v-if="form.isRecommend === 'Y'" v-model="form.reference" v-show-field="['reference', includeFields]" name="reference" label="推荐来源" input-align="left">
        <template #input>
          <TextareaView :value="form.reference" />
        </template>
      </van-field>

      <van-field v-if="!isNil(form.certificates) && form.certificates.includes('9')" v-model="form.otherCertificates" v-show-field="['otherCertificates', includeFields]" name="otherCertificates" label="其他证书" input-align="left">
        <template #input>
          <TextareaView :value="form.otherCertificates" />
        </template>
      </van-field>

      <van-field v-show-field="['transactorOpinion', includeFields]" name="transactorOpinion" label="办理人意见" input-align="left">
        <template #input>
