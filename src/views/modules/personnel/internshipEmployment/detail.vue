<template>
  <van-form ref="Form" v-loading="isLoading && showLoading" class="reset-label" label-width="auto" label-align="top" readonly>
    <van-cell-group inset class="!my-3">
      <van-field v-show-field="['userInfoId', includeFields]" name="userInfoId" label="员工" input-align="left">
        <template #input>
          <UserInfoSelect v-model="form.userInfoId" v-model:user-id="form.userId" v-model:user-name="form.userName" status="-1" readonly />
        </template>
      </van-field>

      <van-field v-show-field="['oldDeptId', includeFields]" name="oldDeptId" label="见习部门" input-align="left">
        <template #input>
          <DeptSelect v-model="form.oldDeptId" readonly />
        </template>
      </van-field>

      <van-field v-show-field="['oldPostId', includeFields]" name="oldPostId" label="见习岗位" input-align="left">
        <template #input>
      ...
      <van-field v-show-field="['specialCommercialInsurance', includeFields]" name="specialCommercialInsurance" label="购买商业保险" input-align="left">
        <template #input>
          <YesNoSwitch v-model="form.specialCommercialInsurance" readonly />
        </template>
      </van-field>

      <van-field v-show-field="['isProbation', includeFields]" name="isProbation" label="是否有试用期" input-align="left">
        <template #input>
          <YesNoSwitch v-model="form.isProbation" readonly />
        </template>
      </van-field>

      <van-field v-if="form.isProbation === 'Y'" v-show-field="['startDate', includeFields]" name="startDate" label="开始日期" input-align="left">
        <template #input>
          {{ parseTime(form.startDate, '{y}-{m}-{d}') }}
        </template>
      </van-field>
      <van-field v-if="form.isProbation === 'Y'" v-show-field="['endDate', includeFields]" name="endDate" label="结束日期" input-align="left">
        <template #input>
      ...
      <van-field v-show-field="['probationWagesRate', includeFields]" name="probationWagesRate" label="试用期薪资发放标准" input-align="left">
        <template #input>
          <span>{{ form.probationWagesRate }}%</span>
        </template>
      </van-field>

      <van-field v-model="form.wages" v-show-field="['wages', includeFields]" name="wages" label="工资" input-align="left">
        <template #input>
          <span class="mr-3">{{ formatCurrency(form.wages) }} </span>
        </template>
      </van-field>

      <van-field v-model="form.baseWages" v-show-field="['baseWages', includeFields]" name="baseWages" label="基本工资" input-align="left">
        <template #input>
          <span class="mr-3">{{ formatCurrency(form.baseWages) }} </span>
        </template>
      </van-field>

      <van-field v-model="form.postWages" v-show-field="['postWages', includeFields]" name="postWages" label="岗位工资" input-align="left">
        <template #input>
          <span class="mr-3">{{ formatCurrency(form.postWages) }} </span>
        </template>
      </van-field>

      <van-field v-model="form.performanceWages" v-show-field="['performanceWages', includeFields]" name="performanceWages" label="绩效工资" input-align="left">
        <template #input>
      ...
      <van-field v-show-field="['formalDate', includeFields]" name="formalDate" label="转正生效日期" input-align="left">
        <template #input>
          {{ parseTime(form.formalDate, '{y}-{m}-{d}') }}
        </template>
      </van-field>

      <!-- <van-field v-show-field="['newDeptId', includeFields]" name="newDeptId" label="见习部门" input-align="left">
      <template #input>
        <DeptSelect v-model="form.newDeptId" readonly />
      </template>
    </van-field>

    <van-field v-show-field="['newPostId', includeFields]" name="newPostId" label="见习岗位" input-align="left">
      <template #input>
        <PostSelect v-model="form.newPostId" :dept-id="form.newDeptId" multiple readonly />
      </template>
    </van-field> -->

      <van-field v-show-field="['humanResourcesOpinion', includeFields]" name="humanResourcesOpinion" label="人力资源部意见" input-align="left">
        <template #input>
          <TextareaView :value="form.humanResourcesOpinion" readonly />
        </template>
      </van-field>

      <van-field v-show-field="['remark', includeFields]" name="remark" label="其他" input-align="left">
        <template #input>
