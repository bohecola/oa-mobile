...
export function useForm() {
  ...
  // 提交
  // const submit = (options?: SubmitOptions<SuccessData>) => {
  ...
  //         else {
  //           if (form.value.id) {
  //             const { data } = await updateUserEmployment(form.value).finally(() => (updateLoading.value = false))
  //           }
  //           else {
