<template>
  <van-form ref="Form" v-loading="isLoading && showLoading" class="reset-label" label-width="auto" label-align="top" readonly>
    <van-cell-group inset class="!my-3">
      <van-field v-show-field="['preEmploymentId', includeFields]" name="preEmploymentId" label="员工" input-align="left">
        <template #input>
      ...
      <van-field v-if="form.level >= 38" v-show-field="['interviewDate', includeFields]" name="interviewDate" label="面试日期" input-align="left">
        <template #input>
          {{ parseTime(form.interviewDate, '{y}-{m}-{d}') }}
        </template>
      </van-field>

      <van-field v-if="form.level >= 38" v-show-field="['interviewWay', includeFields]" name="interviewWay" label="面试形式" input-align="left">
        <template #input>
          <DictSelect v-model="form.interviewWay" dict-type="oa_interview" readonly />
        </template>
      </van-field>

      <van-field v-show-field="['isOutsource', includeFields]" name="isOutsource" label="是否为外包人员" input-align="left">
        <template #input>
          <YesNoSwitch v-model="form.isOutsource" readonly />
        </template>
      </van-field>

      <van-field v-if="form.level >= 38" v-show-field="['isOwnerInterview', includeFields]" name="isOwnerInterview" label="是否需要业主面试" input-align="left">
        <template #input>
      ...
      <van-field v-if="form.level >= 38" v-show-field="['certificates', includeFields]" name="certificates" label="持证情况" input-align="left">
        <template #input>
      ...
      <van-field v-if="form.level >= 38" v-show-field="['employmentEvaluate', includeFields]" name="employmentEvaluate" label="面试评价" input-align="left">
        <template #input>
