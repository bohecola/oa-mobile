<template>
  <van-field v-show-field="['yy_entryCompanyDate', includeFields]" label="入职时间" name="yy_entryCompanyDate">
    <template #input>
      {{ parseTime(form.yy_entryCompanyDate, '{y}-{m}-{d}') }}
    </template>
  </van-field>

  <van-field v-show-field="['yy_subsidyStandards', includeFields]" label="补贴标准(元/月)" name="yy_subsidyStandards">
    <template #input>
  ...
  <van-field v-show-field="['yy_type', includeFields]" label="申请注册/使用证书类型" name="yy_type">
    <template #input>
      <DictSelect v-model="form.yy_type" dict-type="oa_document_type" readonly />
    </template>
  </van-field>

  <van-field v-model="form.yy_name" v-show-field="['yy_name', includeFields]" label="申请注册/使用证书名称" name="yy_name" readonly />

  <van-field v-show-field="['yy_certificateLevel', includeFields]" label="证书等级" name="yy_certificateLevel">
    <template #input>
      <DictSelect v-model="form.yy_certificateLevel" dict-type="oa_certificate_level" readonly />
    </template>
  </van-field>

  <van-field v-show-field="['yy_registrationCompanyDate', includeFields]" label="注册到公司日期" name="yy_registrationCompanyDate">
    <template #input>
      {{ parseTime(form.yy_registrationCompanyDate, '{y}-{m}-{d}') }}
    </template>
  </van-field>

  <van-field v-show-field="['yy_otherCompanyUseStatus', includeFields]" label="证书目前是否正在其他公司使用" name="yy_otherCompanyUseStatus">
    <template #input>
      <YesNoSwitch v-model="form.yy_otherCompanyUseStatus" readonly />
    </template>
  </van-field>

  <van-field v-show-field="['yy_certificateStatus', includeFields]" label="证书状态" name="yy_certificateStatus">
    <template #input>
      <DictSelect v-model="form.yy_certificateStatus" dict-type="sys_normal_disable" readonly />
    </template>
  </van-field>

  <van-field v-model="form.yy_speciality" v-show-field="['yy_speciality', includeFields]" label="专业名称" name="yy_speciality" readonly />

  <van-field v-model="form.yy_no" v-show-field="['yy_no', includeFields]" label="编号" name="yy_no" readonly />
  ...
  <van-field v-model="form.yy_unit" v-show-field="['yy_unit', includeFields]" label="发证单位" name="yy_unit" readonly />

  <van-field v-show-field="['yy_isTraining', includeFields]" label="是否参与培训" name="yy_isTraining">
    <template #input>
