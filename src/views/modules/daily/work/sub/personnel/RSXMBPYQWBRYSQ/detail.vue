<template>
  ...
  <div v-if="form.k_isUniform === 'Y'">
    <van-field v-show-field="['k_workwearType', includeFields]" name="k_workwearType" label="工服类型" input-align="right">
      <template #input>
        <DictSelect v-model="form.k_workwearType" dict-type="oa_daily_work_rsxmbpywbrysq_workwear_type" multiple readonly />
      </template>
    </van-field>

    <van-field v-show-field="['k_quantityAndSize', includeFields]" name="k_quantityAndSize" label="数量与尺寸" input-align="right">
      <template #input>
  ...
  <van-field v-if="['0', '1', '2'].includes(form.k_category)" v-model="form.k_salaryStandards" v-show-field="['k_salaryStandards', includeFields]" label="工资标准（人/元/月）" name="k_salaryStandards" input-align="right" />
  ...
  <van-field v-if="['0', '1', '2', '5'].includes(form.k_category)" v-show-field="['k_specialInstructions', includeFields]" label="工资发放特殊说明" name="k_specialInstructions" input-align="right">
    <template #input>
  ...
  <div v-if="['0', '1', '2', '3', '5'].includes(form.k_category)">
    ...
    <div v-if="['0', '1'].includes(form.k_signeContractType)">
      <van-field v-show-field="['k_signeContractStartDate', includeFields]" label="合同签订开始时间" name="k_signeContractStartDate" input-align="right">
        <template #input>
          {{ parseTime(form.k_signeContractStartDate, '{y}-{m}') }}
        </template>
      </van-field>
      <van-field v-show-field="['k_signeContractEndDate', includeFields]" label="合同签订结束时间" name="k_signeContractEndDate" input-align="right">
        <template #input>
          {{ parseTime(form.k_signeContractEndDate, '{y}-{m}') }}
        </template>
      </van-field>
      <van-field v-show-field="['k_signeContractSpecialInstructions', includeFields]" label="合同签订特殊说明" name="k_signeContractSpecialInstructions" input-align="right">
        <template #input>
    ...
    <div v-if="['0', '1', '2'].includes(form.k_purchaseInsuranceType)">
      <van-field v-show-field="['k_insuranceStartDate', includeFields]" label="保险开始时间" name="k_insuranceStartDate" input-align="right">
        <template #input>
          {{ parseTime(form.k_insuranceStartDate, '{y}-{m}') }}
        </template>
      </van-field>
      <van-field v-show-field="['k_insuranceEndDate', includeFields]" label="保险结束时间" name="k_insuranceEndDate" input-align="right">
        <template #input>
          {{ parseTime(form.k_insuranceEndDate, '{y}-{m}') }}
        </template>
      </van-field>
      <van-field v-show-field="['k_insuranceSpecialInstructions', includeFields]" label="保险购买特殊情况说明" name="k_insuranceSpecialInstructions" input-align="right">
        <template #input>
  ...
  <van-field v-if="['0', '1', '3', '4', '5', '6'].includes(form.k_category)" v-show-field="['k_employInformation', includeFields]" label="聘用人员信息" name="k_employInformation" input-align="right">
    <template #input>
      <TextareaView :value="form.k_employInformation" />
    </template>
  </van-field>

  <van-field v-if="['1', '2', '3', '4'].includes(form.k_category)" v-show-field="['k_changeReason', includeFields]" label="变更原因" name="k_changeReason" input-align="right">
    <template #input>
