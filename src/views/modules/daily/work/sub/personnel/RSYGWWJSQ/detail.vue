<template>
  ...
  <van-field v-show-field="['n_mounth', includeFields]" name="n_mounth" label="申请月份" input-align="right">
    <template #input>
      {{ parseTime(form.n_mounth, '{y}-{m}') }}
    </template>
  </van-field>

  <van-field v-show-field="['n_giftGoldCategory', includeFields]" name="n_giftGoldCategory" label="礼（慰问）金类别" input-align="right">
    <template #input>
      <DictSelect v-model="form.n_giftGoldCategory" dict-type="oa_daily_work_rsygwwjsq_gift_gold_category" multiple readonly />
    </template>
  </van-field>

  <van-field v-show-field="['n_amount', includeFields]" name="n_amount" label="本月礼（慰问）金合计" input-align="right">
    <template #input>
      {{ form.n_amount }}
