<template>
  <van-field v-show-field="['fileUseType', includeFields]" label="使用方式：" prop="fileUseType" name="fileUseType" input-align="right">
    <template #input>
      <DictSelect v-model="form.fileUseType" dict-type="oa_file_use_type" :multiple="false" readonly />
    </template>
  </van-field>

  <van-field v-show-field="['sealFileCategory', includeFields]" label="文件类别：" prop="sealFileCategory" name="sealFileCategory" input-align="right">
    <template #input>
      <DictSelect v-model="form.sealFileCategory" dict-type="oa_seal_file_category" :multiple="false" readonly />
    </template>
  </van-field>

  <van-field v-show-field="['sealUseType', includeFields]" label="用印类型：" prop="sealUseType" name="sealUseType" input-align="right">
    <template #input>
...
withDefaults(
  ...
  {
    includeFields: () => ['fileUseType', 'sealFileCategory', 'sealUseType', 'reason'],
  },
)
